		...
		if (d.async) {
			...
			return {
				send : function(h, i) {
					...
					a += 1;
					e = b(
							'<iframe src="' + c + '" name="iframe-transport-'
									+ a + '"></iframe>').bind(
							"load",
							function() {
								var j, k = b.isArray(d.paramName) ? d.paramName
										: [ d.paramName ];
								e.unbind("load").bind(
										"load",
										function() {
								...
								f.prop("target", e.prop("name")).prop("action",
										d.url).prop("method", d.type);
								if (d.formData) {
									b.each(d.formData, function(l, m) {
										b('<input type="hidden"/>').prop(
												"name", m.name).val(m.value)
												.appendTo(f)
									})
								}
								if (d.fileInput && d.fileInput.length
										&& d.type === "POST") {
									j = d.fileInput.clone();
									...
									f.append(d.fileInput).prop("enctype",
											"multipart/form-data").prop(
								...
								if (j && j.length) {
									d.fileInput.each(function(m, l) {
										var n = b(j[m]);
										b(l).prop("name", n.prop("name")).attr(
												"form", n.attr("form"));
										n.replaceWith(l)
									})
								}
	...
	b.ajaxSetup({
		converters : {
			...
			"iframe xml" : function(c) {
				var d = c && c[0];
				return d && b.isXMLDoc(d) ? d : b
						.parseXML((d.XMLDocument && d.XMLDocument.xml)
								|| b(d.body).html())
			},
			...
			b
					.widget(
							"blueimp.fileupload",
							{
								...
								_specialOptions : [ "fileInput", "dropZone",
										"pasteZone", "multipart",
										"forceIframeTransport" ],
								_blobSlice : b.support.blobSlice
										&& function() {
											var c = this.slice
													|| this.webkitSlice
													|| this.mozSlice;
								...
								_BitrateTimer : function() {
									this.timestamp = ((Date.now) ? Date.now()
											: (new Date()).getTime());
									this.loaded = 0;
									this.bitrate = 0;
								...
								_isXHRUpload : function(c) {
									return !c.forceIframeTransport
											&& ((!c.multipart && b.support.xhrFileUpload) || b.support.xhrFormDataFileUpload)
								},
								...
								_onProgress : function(g, f) {
									if (g.lengthComputable) {
										...
										c = Math
												.floor(g.loaded
														/ g.total
														* (f.chunkSize || f._progress.total))
												+ (f.uploadedBytes || 0);
										this._progress.loaded += (c - f._progress.loaded);
										this._progress.bitrate = this._bitrateTimer
												.getBitrate(d,
														this._progress.loaded,
														f.bitrateInterval);
										f._progress.loaded = f.loaded = c;
										f._progress.bitrate = f.bitrate = f._bitrateTimer
												.getBitrate(d, c,
														f.bitrateInterval);
										this._trigger("progress", b.Event(
												"progress", {
													delegatedEvent : g
												}), f);
										this._trigger("progressall", b.Event(
												"progressall", {
													delegatedEvent : g
												}), this._progress)
									}
								...
								_isInstanceOf : function(c, d) {
									return Object.prototype.toString.call(d) === "[object "
											+ c + "]"
								},
								_initXHRData : function(d) {
									var f = this, h, e = d.files[0], c = d.multipart
											|| !b.support.xhrFileUpload, g = b
											.type(d.paramName) === "array" ? d.paramName[0]
											: d.paramName;
									...
									if (!c || d.blob
											|| !this._isInstanceOf("File", e)) {
										d.headers["Content-Disposition"] = 'attachment; filename="'
												+ encodeURI(e.name) + '"'
									}
									if (!c) {
															...
															.each(
																	...
																	function(i,
																		...
																		h
																				.push({
																					name : (b
																							.type(d.paramName) === "array" && d.paramName[i])
																							|| g,
																					value : j
																				})
																	})
								...
								_initFormSettings : function(c) {
									if (!c.form || !c.form.length) {
										...
										if (!c.form.length) {
											c.form = b(this.options.fileInput
													.prop("form"))
										}
									}
									c.paramName = this._getParamName(c);
									if (!c.url) {
										c.url = c.form.prop("action")
												|| location.href
									}
									c.type = (c.type
											|| (b.type(c.form.prop("method")) === "string" && c.form
													.prop("method")) || "")
											.toUpperCase();
									if (c.type !== "POST" && c.type !== "PUT"
											&& c.type !== "PATCH") {
										c.type = "POST"
									}
									if (!c.formAcceptCharset) {
										c.formAcceptCharset = c.form
												.attr("accept-charset")
									}
								},
								...
								_addConvenienceMethods : function(g, f) {
									var d = this, c = function(e) {
										return b.Deferred().resolveWith(d, e)
												.promise()
									};
									f.process = function(h, e) {
										if (h || e) {
											f._processQueue = this._processQueue = (this._processQueue || c([ this ]))
													.pipe(
															function() {
																if (f.errorThrown) {
																	return b
								...
								_getUploadedBytes : function(e) {
									var c = e.getResponseHeader("Range"), f = c
											&& c.split("-"), d = f
											&& f.length > 1
											&& parseInt(f[1], 10);
									return d && d + 1
								...
								_chunkedUpload : function(n, h) {
									...
									var g = this, e = n.files[0], f = e.size, c = n.uploadedBytes, d = n.maxChunkSize
											|| f, j = this._blobSlice, k = b
											.Deferred(), m = k.promise(), i, l;
									if (!(this._isXHRUpload(n) && j && (c || d < f))
											|| n.data) {
										return false
									}
									if (h) {
									...
									l = function() {
										...
										q.contentRange = "bytes " + c + "-"
												+ (c + q.chunkSize - 1) + "/"
												+ f;
										g._initXHRData(q);
										g._initProgressListener(q);
										i = ((g._trigger("chunksend", null, q) !== false && b
												.ajax(q)) || g._getXHRPromise(
												false, q.context))
												.done(
														function(o, s, r) {
															c = g
								...
								_beforeSend : function(d, c) {
									...
									c._progress.total = c.total = this
											._getTotal(c.files) || 1;
								...
								_onSend : function(i, g) {
									...
									atrapado = this._getAJAXSettings(g)
									
									var h = this, k, c, j, d, l = h
											._getAJAXSettings(g), f = function() {
										h._sending += 1;
										...
										k = k
												|| (((c || h._trigger("send", b
														...
														._getXHRPromise(false,
																l.context, c))
														|| h._chunkedUpload(l) || b
														.ajax(l))
														.done(
																function(e, n,
																		m) {
														...
														.always(
																function(n, o,
																	...
																	h._sending -= 1;
																	h._active -= 1;
																	if (l.limitConcurrentUploads
																			&& l.limitConcurrentUploads > h._sending) {
																		var e = h._slots
																				.shift();
																		while (e) {
																			if (h
																					._getDeferredState(e) === "pending") {
																				e
																						.resolve();
																				break
																			}
								...
								_onAdd : function(q, m) {
									var p = this, v = true, u = b.extend({},
											this.options, m), f = m.files, s = f.length, g = u.limitMultiFileUploads, k = u.limitMultiFileUploadSize, t = u.limitMultiFileUploadSizeOverhead, o = 0, n = this
											._getParamName(u), d, c, r, l, h = 0;
									if (k && (!s || f[0].size === undefined)) {
										k = undefined
									}
									if (!(u.singleFileUploads || g || k)
											|| !this._isXHRUpload(u)) {
										r = [ f ];
										d = [ n ]
									} else {
								...
								_handleFileTreeEntry : function(h, j) {
									var e = this, i = b.Deferred(), d = function(
											l) {
										if (l && !l.entry) {
											l.entry = h
										}
								...
								_getDroppedFiles : function(d) {
									...
									if (c
											&& c.length
											&& (c[0].webkitGetAsEntry || c[0].getAsEntry)) {
										return this
												._handleFileTreeEntries(b
														.map(
																c,
																function(f) {
																	var e;
									...
									return b.Deferred().resolve(
											b.makeArray(d.files)).promise()
								},
								_getSingleFileInputFiles : function(e) {
									e = b(e);
									var c = e.prop("webkitEntries")
											|| e.prop("entries"), d, f;
									if (c && c.length) {
								...
								_onChange : function(f) {
									...
									this
											._getFileInputFiles(d.fileInput)
											.always(
													function(e) {
														d.files = e;
								...
								_onPaste : function(f) {
									var c = f.originalEvent
											...
											&& f.originalEvent.clipboardData.items, d = {
										files : []
									};
									if (c && c.length) {
										...
										if (this._trigger("paste", b.Event(
												"paste", {
													delegatedEvent : f
												}), d) !== false) {
											this._onAdd(f, d)
								...
								_initSpecialOptions : function() {
									...
									if (c.fileInput === undefined) {
										c.fileInput = this.element
												.is('input[type="file"]') ? this.element
												: this.element
								...
								add : function(d) {
									...
									if (d.fileInput && !d.files) {
										this._getFileInputFiles(d.fileInput)
												.always(function(e) {
													d.files = e;
													c._onAdd(null, d)
												})
								...
								send : function(g) {
									...
									return this._getXHRPromise(false, g
											&& g.context)
								}
							})
		}));
PrimeFaces.widget.FileUpload = PrimeFaces.widget.BaseWidget
		.extend({
			...
			init : function(a) {
				...
				this.renderMessages();
				this.bindEvents();
				var c = this, e = this.form.attr("action"), d = this.form
				...
				this.ucfg = {
					...
					add : function(l, k) {
						...
						if (c.cfg.fileLimit
								&& (c.uploadedFileCount + c.files.length + 1) > c.cfg.fileLimit) {
							c.clearMessages();
							c.showMessage({
						...
						if (h) {
							...
							if (m) {
								...
								var o = $("<tr></tr>")
										...
										.append("<td>" + h.name + "</td>")
										.append(
												"<td>" + c.formatSize(h.size)
														+ "</td>")
										.append(
												'<td class="ui-fileupload-progress"></td>')
										.append(
												'<td><button class="ui-fileupload-cancel ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only"><span class="ui-button-icon-left ui-icon ui-icon ui-icon-close"></span><span class="ui-button-text">ui-button</span></button></td>')
										.appendTo(c.filesTbody);
								if (c.isCanvasSupported() && window.File
										&& window.FileReader
										&& c.IMAGE_TYPES.test(h.name)) {
									var n = $("<canvas></canvas>")
											.appendTo(
													o
															.children("td.ui-fileupload-preview")), g = n
											.get(0).getContext("2d"), i = window.URL
											|| window.webkitURL, f = i
											.createObjectURL(h), j = new Image();
									j.onload = function() {
								...
								o
										.children("td.ui-fileupload-progress")
										.append(
					...
					progress : function(k, j) {
						...
						PrimeFaces.ajax.Response.handle(f.result, f.textStatus,
								f.jqXHR, null)
					},
					always : function(g, f) {
						if (c.cfg.oncomplete) {
			...
			bindEvents : function() {
				...
				this.chooseButton.on("mouseover.fileupload", function() {
					...
					$(this).removeClass("ui-state-active ui-state-hover")
						...
						function() {
							var b = $(this);
							if (!b.prop("disabled")) {
								b.addClass("ui-state-active").removeClass(
										"ui-state-hover")
							}
						}).on(
						"mouseup.fileupload",
				...
				this.chooseButton.children("input").on("focus.fileupload",
						function() {
				...
				this.uploadButton.on("click.fileupload", function(b) {
					...
					var comprobador = window[a.widgetVar + 'Comprobar'];
					var refrescar = window[a.widgetVar + 'Refrescar'];
					var evento = {};
					evento.termino = true;
					evento.contexto = a;
					var _a = 0;
					...
					function subida()
					{
						if (evento.termino && _a < a.files.length)
						{
							evento.termino = false;
							comprobarArchivo(a.files[_a], comprobador, evento);
							_a++;
						}
						else if (_a == a.files.length)
						{
							refrescar();
							return;
						}
						
						setTimeout(subida, 100);
					}					
				...
				$(document)
						.off(
								...
								null,
								function(c) {
									var d = $(this).closest("tr"), b = a.files
											.splice(d.index(), 1);
									b[0].row = null;
									a.removeFileRow(d);
									if (a.files.length === 0) {
